...
const electionDataUrl =
  "https://data.gov.il/api/3/action/datastore_search?resource_id=929b50c6-f455-4be2-b438-ec6af01421f2";
let myChart;
const filteredKeys = [
  "מצביעים",
  "rank שם ישוב",
...
async function getCityResult() {
  ...
  const response = await fetch(
    `${electionDataUrl}&q={"שם ישוב":"${select.value}"}`
  );
  const data = await response.json();
